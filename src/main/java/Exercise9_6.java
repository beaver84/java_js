...
*/
        String fillZeroResult = "";
        String result = "";
        if (length > 1 && src.length() > length) {
            fillZeroResult = src.substring(0, length);
        } else if (length <= 0) {
            fillZeroResult = "";
        } else if (src.length() < length) {
            int fillZeroLength = length - src.length();
            for (int i = 0; i < fillZeroLength; i++) {
                result = result.concat("0");
            }
            fillZeroResult = result + src;
        }
        return fillZeroResult;
    }
